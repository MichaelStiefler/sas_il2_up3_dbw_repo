    ...
    private double dpythag(double d, double d1)
    {
        ...
        if(d2 > d3)
        {
            if(d2 == 0.0D)
                return 0.0D;
            double d4 = d3 / d2;
            if(java.lang.Math.abs(d4) <= java.lang.Double.MIN_VALUE)
                return d2;
            else
        ...
        if(d3 == 0.0D)
            return 0.0D;
        double d5 = d2 / d3;
        if(java.lang.Math.abs(d5) <= java.lang.Double.MIN_VALUE)
            return d3;
        else
        ...
        for(int l = 0; l < i; l++)
        {
            for(int j1 = 0; j1 < l; j1++)
            {
                ...
                for(int k1 = 0; k1 < j1; k1++)
                    if(ad[j1 * i + k1] != 0.0D && ad[k1 * i + l] != 0.0D)
                        d3 -= ad[j1 * i + k1] * ad[k1 * i + l];
            ...
            for(int l1 = l; l1 < i; l1++)
            {
                ...
                for(int i2 = 0; i2 < l; i2++)
                    if(ad[l1 * i + i2] != 0.0D && ad[i2 * i + l] != 0.0D)
                        d4 -= ad[l1 * i + i2] * ad[i2 * i + l];
