...
public class BombTIPink extends Bomb
{
  private long t1;
  private int charge;
    ...
    if (this.charge < 1.01D) {
      ...
      if (Actor.isValid(localActor)) {
        ...
        for (int i = 0; i < 60; i++) {
          ...
          BombTiPk localBombTiPk = new BombTiPk();
          localBombTiPk.pos.setUpdateEnable(true);
          localBombTiPk.pos.setAbs(localPoint3d, localOrient);
          localBombTiPk.pos.reset();
          localBombTiPk.start();
          localBombTiPk.setOwner(localActor, false, false, false);
          localBombTiPk.setSpeed(localVector3d1);
        }
  ...
  static
  {
    Class localClass = BombTIPink.class;
