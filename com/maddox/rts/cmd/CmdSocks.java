...
public class CmdSocks extends com.maddox.rts.Cmd
{
    ...
    {
        boolean flag = true;
        if(com.maddox.rts.cmd.CmdSocks.exist(map, "ON"))
        {
            com.maddox.rts.net.SocksUdpSocket.setProxyEnable(true);
            flag = false;
        } else
        if(com.maddox.rts.cmd.CmdSocks.exist(map, "OFF"))
        {
            com.maddox.rts.net.SocksUdpSocket.setProxyEnable(false);
            flag = false;
        }
        if(com.maddox.rts.cmd.CmdSocks.exist(map, "HOST"))
        {
            if(com.maddox.rts.cmd.CmdSocks.nargs(map, "HOST") > 0)
                com.maddox.rts.net.SocksUdpSocket.setProxyHost(com.maddox.rts.cmd.CmdSocks.arg(map, "HOST", 0));
            else
                com.maddox.rts.net.SocksUdpSocket.setProxyHost(null);
            flag = false;
        }
        if(com.maddox.rts.cmd.CmdSocks.exist(map, "PORT") && com.maddox.rts.cmd.CmdSocks.nargs(map, "PORT") > 0)
        {
            com.maddox.rts.net.SocksUdpSocket.setProxyPort(com.maddox.rts.cmd.CmdSocks.arg(map, "PORT", 0, 1080));
            flag = false;
        }
        if(com.maddox.rts.cmd.CmdSocks.exist(map, "USER"))
        {
            if(com.maddox.rts.cmd.CmdSocks.nargs(map, "USER") > 0)
                com.maddox.rts.net.SocksUdpSocket.setProxyUser(com.maddox.rts.cmd.CmdSocks.arg(map, "USER", 0));
            else
                com.maddox.rts.net.SocksUdpSocket.setProxyUser(null);
            flag = false;
        }
        if(com.maddox.rts.cmd.CmdSocks.exist(map, "PWD"))
        {
            if(com.maddox.rts.cmd.CmdSocks.nargs(map, "PWD") > 0)
                com.maddox.rts.net.SocksUdpSocket.setProxyPassword(com.maddox.rts.cmd.CmdSocks.arg(map, "PWD", 0));
            else
