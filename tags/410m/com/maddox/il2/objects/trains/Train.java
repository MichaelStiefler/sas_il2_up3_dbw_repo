...
public class Train extends com.maddox.il2.ai.Chief
{
    ...
    public void BridgeSegmentDestroyed(int i, int j, com.maddox.il2.engine.Actor actor)
    {
        ...
        if(!flag)
            return;
        int k;
        for(k = tailSeg; k <= headSeg && !road.segIsWrongOrDamaged(k); k++);
        if(k > headSeg)
            return;
    ...
    private void moveTrain(float f)
    {
        ...
        do
        {
            if(headAlong + d1 < roadsegment.length2D)
                break;
    ...
    {
        int i;
        for(i = 0; i < wagons.size() && wagon != (com.maddox.il2.objects.trains.Wagon)wagons.get(i); i++);
        if(i >= wagons.size())
