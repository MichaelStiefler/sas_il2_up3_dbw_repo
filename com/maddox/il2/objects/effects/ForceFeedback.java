    ...
    public static void fxTriggerShake(int i, boolean flag)
    {
        ...
        if(triggerFXType[i] == 0)
        {
            ...
            for(int j = 0; j < abulletemitter.length; j++)
            {
                ...
                {
                    com.maddox.il2.engine.GunGeneric gungeneric = (com.maddox.il2.engine.GunGeneric)abulletemitter[j];
                    if(gungeneric.prop == null)
                        return;
                    float f = gungeneric.prop.shotFreq;
                    if(f > 10.83F)
                    {
                ...
                if(!(abulletemitter[j] instanceof com.maddox.il2.objects.weapons.RocketBombGun))
                    continue;
                triggerFX[i] = new com.maddox.rts.JoyFF.Effect("ForceFeedback/bomb.ffe");
                triggerFXType[i] = 5;
                break;
