...
public class Motor extends com.maddox.il2.fm.FMMath
{
    ...
    private void resolveFromFile(com.maddox.rts.SectFile sectfile, java.lang.String s)
    {
        ...
        if(f != -99999F)
        {
            ...
            if(propAngleDeviceType == 1 || propAngleDeviceType == 2 || propAngleDeviceType == 7 || propAngleDeviceType == 8)
    ...
    private void calcAfterburnerCompressorFactor()
    {
        if(afterburnerType == 1 || afterburnerType == 7 || afterburnerType == 8 || afterburnerType == 10 || afterburnerType == 11 || afterburnerType == 6 || afterburnerType == 5 || afterburnerType == 9 || afterburnerType == 4)
        {
            ...
            } else
            {
                ...
                {
                    for(int k = 0; k < nOfCompPoints - 1; k++)
                    {
    ...
    private void computeForces(float f)
    {
        ...
        case 7: // '\007'
            ...
            case 7: // '\007'
                ...
                if(engineBoostFactor > 1.0F)
                    f18 = 0.9090909F * controlThrottle;
                float f16 = propAngleDeviceMaxParam;
                if(controlAfterburner)
                    if(afterburnerType == 1)
                    {
                        if(controlThrottle > 1.0F)
                            f16 = propAngleDeviceMaxParam + 10F * (controlThrottle - 1.0F) * (propAngleDeviceAfterburnerParam - propAngleDeviceMaxParam);
                    } else
                    {
                        f16 = propAngleDeviceAfterburnerParam;
                    }
                float f2 = propAngleDeviceMinParam + (f16 - propAngleDeviceMinParam) * f18;
                float f11 = controlThrottle;
                if(f11 > 1.0F)
                    f11 = 1.0F;
                compressorManifoldThreshold = getATA(toRPM(propAngleDeviceMinParam + (f16 - propAngleDeviceMinParam) * f11));
                if(isPropAngleDeviceOperational())
                    if(bControlPropAuto)
                    {
                        if(w < f2)
            ...
            case 8: // '\b'
                float f19 = controlThrottle;
                if(engineBoostFactor > 1.0F)
                    f19 = 0.9090909F * controlThrottle;
                ...
                float f3 = propAngleDeviceMinParam + (f17 - propAngleDeviceMinParam) * f19 + (bControlPropAuto ? 0.0F : -25F + 50F * controlProp);
                float f12 = controlThrottle;
                if(f12 > 1.0F)
                    f12 = 1.0F;
                compressorManifoldThreshold = getATA(toRPM(propAngleDeviceMinParam + (f17 - propAngleDeviceMinParam) * f12));
                if(isPropAngleDeviceOperational())
                {
                    if(w < f3)
                    {
            ...
            if(propPhi < propPhiMin)
                propPhi = propPhiMin;
            engineMoment = getN();
            float f21 = getCompressorMultiplier(f);
            engineMoment *= f21;
        ...
        case 2: // '\002'
            ...
            float f29 = f27 * f27;
            float f30 = 1.0F - 0.006F * (com.maddox.il2.fm.Atmosphere.temperature((float)reference.Loc.z) - 290F);
        ...
        case 6: // '\006'
            ...
            if(w < wMin || w < 0.0F || stage != 6)
                w = 0.0F;
            float f32 = reference.getSpeed() / 94F;
            if(f32 < 1.0F)
                w = 0.0F;
            else
                f32 = (float)java.lang.Math.sqrt(f32);
            propForce = (w / wMax) * thrustMax * f32;
            propForce *= getStageMultiplier();
    ...
    public float forcePropAOA(float f, float f1, float f2, boolean flag)
    {
        ...
        case 7: // '\007'
            ...
            do
            {
                ...
                if((propForce <= 0.0F || java.lang.Math.abs(propMoment - f4) >= 1E-005F) && j <= 32)
                {
                ...
                {
                    float f9 = (float)java.lang.Math.toDegrees(java.lang.Math.atan(f / (wMax * propReductor * propr)) + (double)f8);
                    return propForce;
                }
            } while(true);
        ...
        case 2: // '\002'
            ...
            pressureExtBar /= com.maddox.il2.fm.Atmosphere.P0();
            float f10 = pressureExtBar;
            float f11 = 1.0F - 0.006F * (com.maddox.il2.fm.Atmosphere.temperature(f1) - 290F);
            float f12 = 1.0F - 0.0011F * f;
            propForce = thrustMax * f10 * f11 * f12;
            return propForce;
    ...
    public float getN()
    {
        if(stage == 6)
        {
            ...
            case 0: // '\0'
                ...
                float f3 = w / wMax;
                tmpF = engineMomentMax * ((-1F / f) * f3 * f3 + 2.0F * f3);
            ...
            case 3: // '\003'
                ...
                if(getControlThrottle() > 1.0F)
                    tmpF *= engineBoostFactor;
                if(reference.getOverload() < 0.0F)
                {
                    producedDistabilisation += 16F;
                    tmpF *= 0.5F;
                    if(reference.isPlayers() && (reference instanceof com.maddox.il2.fm.RealFlightModel) && ((com.maddox.il2.fm.RealFlightModel)reference).isRealMode() && bIsMaster && com.maddox.il2.ai.World.Rnd().nextFloat() < 0.001F)
                        setEngineStops(reference.actor);
                }
                overrevving();
                break;
            ...
            case 2: // '\002'
                ...
                if(f2 > 1.0F)
                    f2 = 1.0F;
                float f6 = engineMomentMax * (-0.5F * f2 * f2 + 1.0F * f2 + 0.5F);
                float f5;
                if(controlAfterburner)
                    f5 = w / (wWEP * f2);
                else
                    f5 = w / (wNom * f2);
                tmpF = f6 * (2.0F * f5 - 1.0F * f5 * f5);
                if(getControlThrottle() > 1.0F)
