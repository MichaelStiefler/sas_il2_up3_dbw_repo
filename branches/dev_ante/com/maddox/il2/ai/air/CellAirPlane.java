  ...
  implements Serializable
{
  public double ofsX;
  public double ofsY;
  private int iFoldedWidth = -1;

  public void setFoldedWidth(int paramInt)
  {
    this.iFoldedWidth = paramInt;
  }

  public int getWidth()
  {
    if (this.iFoldedWidth > 0) {
      return this.iFoldedWidth;
    }
  ...
  {
    if (paramCellAirField.getWidth() < getWidth())
      return false;
    return paramCellAirField.getHeight() >= getHeight();
  }

  public boolean checkAirFieldSizeCarrier(CellAirField paramCellAirField)
  {
    if (paramCellAirField.getWidth() < getWidth())
      return false;
    return paramCellAirField.getHeight() >= getHeight() + 1;
  }
  ...
  public void blurSiluet4x()
  {
    ...
    for (int i = 0; i < getHeight(); i++)
    {
      for (int j = 0; j < getWidth(); j++) {
        ...
        if (j > 0)
          localCellObject.getCells()[(j - 1)][i] = localCellObject;
        if (j < getWidth() - 1)
          localCellObject.getCells()[(j + 1)][i] = localCellObject;
        if (i > 0)
          localCellObject.getCells()[j][(i - 1)] = localCellObject;
        if (i < getHeight() - 1) {
          localCellObject.getCells()[j][(i + 1)] = localCellObject;
        }
      }
  ...
  public void clampCells()
  {
    ...
    for (int m = 0; (m < i) && (k == 0); m++)
    {
      for (n = 0; n < j; n++)
      {
        if (arrayOfCellObject1[n][m] == null)
          continue;
        k = 1;
        break;
      }
    ...
    for (int n = i - 1; (n > 0) && (k == 0); n--)
    {
      for (i1 = 0; i1 < j; i1++)
      {
        if (arrayOfCellObject1[i1][n] == null)
          continue;
        k = 1;
        break;
      }
    ...
    for (int i1 = 0; (i1 < j) && (k == 0); i1++)
    {
      for (i2 = 0; i2 < i; i2++)
      {
        if (arrayOfCellObject1[i1][i2] == null)
          continue;
        k = 1;
        break;
      }
    ...
    for (int i2 = j - 1; (i2 > 0) && (k == 0); i2--)
    {
      for (int i3 = 0; i3 < i; i3++)
      {
        if (arrayOfCellObject1[i2][i3] == null)
          continue;
        k = 1;
        break;
      }
  ...
  public void initCellsThroughCollision(HierMesh paramHierMesh, Loc paramLoc, double paramDouble)
  {
    ...
    for (int j = 0; j < i; j++)
    {
      for (int k = 0; k < i; k++)
      {
        double d1 = paramLoc.getY() + (i / 2 - j) * paramDouble - paramDouble * 0.5D;
        double d2 = paramLoc.getX() + (i / 2 - k) * paramDouble - paramDouble * 0.5D;
        int m = 0;
        for (int n = -1; (n <= 1) && (m == 0); n++)
        {
          for (int i1 = -1; (i1 <= 1) && (m == 0); i1++)
          {
            localPoint3d1.set(d2 + n * paramDouble * 0.4D, d1 + i1 * paramDouble * 0.4D, paramLoc.getZ() + paramHierMesh.collisionR());
            localPoint3d2.set(d2 + n * paramDouble * 0.4D, d1 + i1 * paramDouble * 0.4D, paramLoc.getZ() - paramHierMesh.collisionR());
