...
public class ZutiSupportMethods
{
	...
	public static void zutiAssignMarkersToActors()
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			if( nearestActor != null )
			{
				String actorName = nearestActor.toString();
				String makerName = marker.toString();
				actorName = actorName.substring(actorName.lastIndexOf("$")+1, actorName.length()-1);
				makerName = makerName.substring(makerName.lastIndexOf("$")+1, makerName.length()-1);
				
				System.out.println("Marker >" + makerName + "< assigned to >" + nearestActor.name() + "< - " + actorName);
			}
		...
		for( int i=0; i<size; i++ )
		{
			...
			if( bp.zutiEnableFriction && !ZutiSupportMethods.zutiIsOnExistingFrictionArea(bp) )
			{
				...
				if( zaps != null )
				{
					...
					System.out.println("Home base x=" + (int)bp.place.x + ", y=" + (int)bp.place.y + " is RRR area. Friction=" + friction);
				}
				else
	...
	public static boolean isResourcesManagementValid(Point3d pos)
	{
		...
		//System.out.println("Checking resource management for location x=" + (int)pos.x + ", y=" + (int)pos.y);
		BornPlace bp = ZutiSupportMethods_Net.getNearestBornPlace_AnyArmy(pos.x, pos.y);
		if( bp != null && bp.zutiEnableResourcesManagement )
		{
			//System.out.println("  Resources management is ENABLED for HB at x=" + (int)bp.place.x + ", y=" + (int)bp.place.y);
			return true;
		}
		
		int army = Front.army(pos.x, pos.y);;
		if( army == 1 && Mission.MDS_VARIABLES().enabledResourcesManagement_RedSide )
		{
			//System.out.println("  Resources management is ENABLED for RED side!");
			return true;
		}
		else if( army == 2 && Mission.MDS_VARIABLES().enabledResourcesManagement_BlueSide )
		{
			//System.out.println("  Resources management is ENABLED for BLUE side!");
			return true;
		}
	...
	/**
	 * Returns true if location is valid, else false.
	 * @param loc
	 * @return
	 */
	public static boolean isLocValid(Loc loc)
	{
		if (loc == null)
			return false;
