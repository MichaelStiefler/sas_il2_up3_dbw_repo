    ...
    implements java.lang.Cloneable, java.io.Serializable
{
    ...
    public boolean containsValue(java.lang.Object obj)
    {
        ...
        if(obj == null)
        {
            ...
            {
                for(com.maddox.util.Entry entry = aentry[i]; entry != null; entry = entry.next)
                    if(entry.value == null)
                        return true;
    ...
    public int findNotUsedKey(int i, int j, int k)
    {
        ...
        if(i < k)
        {
            ...
            for(int l = j; l < k; l++)
            {
                ...
                for(entry = aentry[(l & 0x7fffffff) % aentry.length]; entry != null; entry = entry.next)
                    if(entry.key == l)
                        break;
            ...
            for(int j1 = i; j1 < j; j1++)
            {
                ...
                for(entry2 = aentry[(j1 & 0x7fffffff) % aentry.length]; entry2 != null; entry2 = entry2.next)
                    if(entry2.key == j1)
                        break;
        ...
        if(i > k)
        {
            ...
            for(int i1 = j; i1 > k; i1--)
            {
                ...
                for(entry1 = aentry[(i1 & 0x7fffffff) % aentry.length]; entry1 != null; entry1 = entry1.next)
                    if(entry1.key == i1)
                        break;
            ...
            for(int k1 = i; k1 > j; k1--)
            {
                ...
                for(entry3 = aentry[(k1 & 0x7fffffff) % aentry.length]; entry3 != null; entry3 = entry3.next)
                    if(entry3.key == k1)
                        break;
    ...
    private void rehash()
    {
        ...
        for(int k = i; k-- > 0;)
        {
            for(com.maddox.util.Entry entry = aentry[k]; entry != null;)
            {
