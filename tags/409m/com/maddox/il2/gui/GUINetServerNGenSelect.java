...
public class GUINetServerNGenSelect extends com.maddox.il2.game.GameState
{
    ...
    private void fillCampList()
    {
        ...
        for(int i = 0; i < as.length; i++)
        {
            ...
            {
                j = s4.length() - s2.length();
                flag = true;
            }
            if(j == -1 && s4.length() > s3.length() && s4.regionMatches(true, s4.length() - s3.length(), s3, 0, s3.length()))
            {
                j = s4.length() - s3.length();
            ...
            if(j >= s1.length())
            {
                java.lang.String s7 = s4.substring(s1.length(), j);
        ...
        if(afile != null && afile.length > 0)
        {
            for(int k = 0; k < afile.length; k++)
                if(afile[k].isDirectory() && !afile[k].isHidden())
                    try
                    {
                        java.io.File file1 = new File(afile[k], "conf.dat");
                        if(file1.exists())
                        {
                            com.maddox.il2.gui.Item item2 = new Item();
                            item2.bNew = false;
                            item2.fileName = "missions/net/ngen/" + afile[k].getName().toLowerCase() + "/conf.dat";
        ...
        {
            int l;
            for(l = 0; l < wTable.campList.size(); l++)
            {
                com.maddox.il2.gui.Item item = (com.maddox.il2.gui.Item)wTable.campList.get(l);
                if(cur.equals(item))
                    break;
            }

            if(l < wTable.campList.size())
