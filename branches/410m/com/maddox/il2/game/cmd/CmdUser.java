...
public class CmdUser extends com.maddox.rts.Cmd
{
    ...
    public java.lang.Object exec(com.maddox.rts.CmdEnv cmdenv, java.util.Map map)
    {
        ...
        if(arraylist.size() == 0 && !com.maddox.il2.game.cmd.CmdUser.exist(map, "STAT"))
            return null;
        bEventLog = com.maddox.il2.game.cmd.CmdUser.exist(map, "EVENTLOG");
        if(com.maddox.il2.game.cmd.CmdUser.exist(map, "STAT"))
        {
    ...
    private void fillUsers(java.util.Map map, java.util.List list, boolean flag)
    {
        ...
        if(!map.containsKey("_$$") && !map.containsKey("#") && !map.containsKey("ARMY"))
            flag1 = true;
        if(map.containsKey("_$$") && com.maddox.il2.game.cmd.CmdUser.nargs(map, "_$$") == 1 && "*".equals(com.maddox.il2.game.cmd.CmdUser.arg(map, "_$$", 0)))
            flag1 = true;
        if(flag1)
        {
            if(flag)
        ...
        {
            int j = com.maddox.il2.game.cmd.CmdUser.nargs(map, "_$$");
label0:
            for(int i1 = 0; i1 < j; i1++)
            {
                java.lang.String s = com.maddox.il2.game.cmd.CmdUser.arg(map, "_$$", i1);
                if(flag)
                {
                    int i2 = 0;
                    do
                    {
                        if(i2 >= com.maddox.il2.net.NetUserLeft.all.size())
                            continue label0;
                ...
                do
                {
                    if(k2 >= com.maddox.rts.NetEnv.hosts().size())
                        continue label0;
        ...
        if(map.containsKey("#"))
        {
            int k = com.maddox.il2.game.cmd.CmdUser.nargs(map, "#");
            for(int j1 = 0; j1 < k; j1++)
            {
                java.lang.String s1 = com.maddox.il2.game.cmd.CmdUser.arg(map, "#", j1);
                if(s1.charAt(0) < '0' || s1.charAt(0) > '9')
                    continue;
                int j2 = com.maddox.il2.game.cmd.CmdUser.arg(map, "#", j1, 1000, 0, 1000);
                if(flag)
                {
        ...
        {
            int l = com.maddox.il2.game.cmd.CmdUser.nargs(map, "ARMY");
label1:
            for(int k1 = 0; k1 < l; k1++)
            {
                int l1 = -1;
                java.lang.String s2 = com.maddox.il2.game.cmd.CmdUser.arg(map, "ARMY", k1);
                if(s2.charAt(0) >= '0' && s2.charAt(0) <= '9')
                {
                    l1 = com.maddox.il2.game.cmd.CmdUser.arg(map, "ARMY", k1, 1000, 0, 1000);
                    if(l1 >= com.maddox.il2.ai.Army.amountNet())
                        continue;
                } else
                {
                    for(l1 = 0; l1 < com.maddox.il2.ai.Army.amountNet() && !com.maddox.il2.ai.Army.name(l1).equals(s2); l1++);
                    if(l1 == com.maddox.il2.ai.Army.amountNet())
                        continue;
                }
                if(flag)
                {
                    ...
                    do
                    {
                        if(i3 >= com.maddox.il2.net.NetUserLeft.all.size())
                            continue label1;
                ...
                do
                {
                    if(j3 >= com.maddox.rts.NetEnv.hosts().size())
                        continue label1;
