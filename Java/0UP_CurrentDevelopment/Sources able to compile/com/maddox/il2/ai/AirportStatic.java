...
public abstract class AirportStatic extends com.maddox.il2.ai.Airport
{
    ...
    public static void make(java.util.ArrayList arraylist, com.maddox.JGP.Point2f point2fs[][], com.maddox.JGP.Point2f point2fs_0_[][], com.maddox.JGP.Point2f point2fs_1_[][])
    {
        if(arraylist != null)
        {
            ...
            while(arraylist.size() > 0) 
            {
                ...
                if(bool)
                {
                    airportstatic.myRunway.add(new Runway(loc));
                    int i = airportstatic.myRunway.size();
                    p3d.set(0.0D, 0.0D, 0.0D);
                    for(int i_3_ = 0; i_3_ < i; i_3_++)
                    {
                        loc = ((Runway)airportstatic.myRunway.get(i_3_)).loc;
    ...
    public void setTakeoff(com.maddox.JGP.Point3d point3d, com.maddox.il2.objects.air.Aircraft aircrafts[])
    {
        ...
        if(runway != null)
        {
            ...
            for(int i = 0; i < aircrafts.length; i++)
                if(com.maddox.il2.engine.Actor.isValid(aircrafts[i]))
                {
                    float f = aircrafts[i].collisionR() * 2.0F + 20F;
                    ...
                    for(int i_9_ = 0; i_9_ < runway.curPlaneShift; i_9_++)
                        if(com.maddox.il2.engine.Actor.isValid(runway.planes[i_9_]))
                        {
                            ...
                            ((com.maddox.il2.fm.FlightModelMain) (((com.maddox.il2.objects.sounds.SndAircraft) (runway.planes[i_9_])).FM)).AP.way.takeoffAirport = this;
                        }

                }

            if(com.maddox.il2.engine.Actor.isValid(aircrafts[0]) && (((com.maddox.il2.objects.sounds.SndAircraft) (aircrafts[0])).FM instanceof com.maddox.il2.ai.air.Maneuver))
            {
    ...
    {
        tmpLoc.set(((com.maddox.il2.fm.FlightModelMain) (flightmodel)).Loc);
    ...
    private Runway nearestRunway(com.maddox.JGP.Point3d point3d)
    {
        Runway runway = null;
        double d = 0.0D;
        np.set(point3d);
        int i = this.myRunway.size();
        for(int i_11_ = 0; i_11_ < i; i_11_++)
        {
            Runway runway_12_ = (Runway)this.myRunway.get(i_11_);
    ...
    private Runway oppositeRunway(com.maddox.il2.engine.Loc loc)
    {
        int i = this.myRunway.size();
        for(int i_14_ = 0; i_14_ < i; i_14_++)
        {
            Runway runway = (Runway)this.myRunway.get(i_14_);
    ...
    private java.util.ArrayList myRunway;
