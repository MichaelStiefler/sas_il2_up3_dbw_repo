...
// <editor-fold defaultstate="collapsed" desc="Constructors">
  ...
  public GuidedMissileUtils(
          ...
          float theMinPkForAttack,
          long theMillisecondsBetweenMissileLaunchAI,
          long theTargetType,
          boolean theAttackDecisionByAI,
          boolean theMultiTrackingCapable,
          boolean theCanTrackSubs,
          String theLockFx,
          String theNoLockFx,
          String theLockSmpl,
    ...
    this.initParams(
            ...
            theMaxDistance,
            theMinPkForAttack,
            theMillisecondsBetweenMissileLaunchAI,
            theTargetType,
            theAttackDecisionByAI,
            theMultiTrackingCapable,
            theCanTrackSubs,
            theLockFx,
            theNoLockFx,
            theLockSmpl,
      ...
      for (int l = 0; l < ownerAircraft.FM.CT.Weapons.length; l++)
        if (ownerAircraft.FM.CT.Weapons[l] != null)
          for (int l1 = 0; l1 < ownerAircraft.FM.CT.Weapons[l].length; l1++) {
            if ((ownerAircraft.FM.CT.Weapons[l][l1] == null) || 
              (!(ownerAircraft.FM.CT.Weapons[l][l1] instanceof GunNull))) continue;
...
//      com.maddox.il2.game.HUD.log("Remains: " + this.rocketsList.size());
  ...
  private void checkAIlaunchMissile() {
    if (!(this.missileOwner instanceof Aircraft)) return;
    Aircraft ownerAircraft = (Aircraft)this.missileOwner;
    if ((((ownerAircraft.FM instanceof RealFlightModel)) && 
      (((RealFlightModel)ownerAircraft.FM).isRealMode())) || 
      (!(ownerAircraft.FM instanceof Pilot))) {
      return;
    }
...
//      HUD.training("trgtPk="+trgtPk);
      if ((this.trgtPk > this.getMinPkForAttack()) && 
        (Actor.isValid(this.getMissileTarget())) && 
        (this.getMissileTarget().getArmy() != ownerAircraft.FM.actor.getArmy()) && 
...
//    this.rocketsList.remove(0);
  ...
  public void update() {
    this.setMissileTarget(this.lookForGuidedMissileTarget(this.missileOwner, this.getMaxPOVfrom(), this.getMaxPOVto(), 
      this.getPkMaxDist(), this.targetType));
    this.trgtPk = getMissilePk();
...
//    }
  ...
  public Actor lookForGuidedMissileTarget(Actor actor, float maxFOVfrom, float maxFOVto, double maxDistance, long targetType) {
    ...
    if ((actorTarget == null) && (targetType & Missile.TARGET_GROUND) != 0) {
      actorTarget = lookForGuidedMissileTargetGround(actor, maxFOVfrom, maxFOVto, maxDistance);
    }
    if ((actorTarget == null) && (targetType & Missile.TARGET_SHIP) != 0) {
    ...
    try {
      ...
      for (int i1 = 0; i1 < k; i1++) {
        ...
        if ((theTarget1 instanceof Aircraft)
                || (theTarget1 instanceof MissileInterceptable)
                 || (theTarget1 instanceof RocketryRocket)) {
          ...
          switch (this.iDetectorMode) {
            case Missile.DETECTOR_TYPE_INFRARED: {
              ...
              } else if (theTarget1 instanceof RocketryRocket) {
                maxEngineForce = 1000F;
              }
  ...
  public Actor lookForGuidedMissileTargetGround(Actor actor, float maxFOVfrom, float maxFOVto, double maxDistance) {
    ...
    try {
      ...
      for (int i1 = 0; i1 < k; i1++) {
        Actor theTarget1 = (Actor) list.get(i1);
        if (    (theTarget1 instanceof TgtFlak)
             || (theTarget1 instanceof TgtTank)
             || (theTarget1 instanceof TgtTrain)
             || (theTarget1 instanceof TgtVehicle) ) {
