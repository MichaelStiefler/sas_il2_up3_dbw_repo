...
import com.maddox.il2.objects.air.F_86D;
import com.maddox.il2.objects.air.F_86K;
import com.maddox.il2.objects.air.F84G1;
import com.maddox.il2.objects.air.F9F;
//import com.maddox.il2.objects.air.FI_156C;
...
//import com.maddox.il2.objects.air.MS_500;
//import com.maddox.il2.objects.air.MS_502;
//import com.maddox.il2.objects.air.MS_505;
//import com.maddox.il2.objects.air.OKA_38;
//import com.maddox.il2.objects.air.L_5;
...
// PAS++
public class AircraftHotKeys extends AircraftHotKeysAlt
...
// PAS--
					...
					case 131: // '\025'
						...
						break;
				        case 135:
				        	//TODO:Placeholder for 'Toggle Weapon System'. For use with missiles and LABS
				        break;
				        case 136:
				        	//TODO:Placeholder for 'Deploy Countermeasures'
				        break;
				        case 137:
				        	//TODO:'Weapon Salvo Size'
				        	if((FM.CT.Weapons[2] != null) || (FM.CT.Weapons[3] != null))
				        	{
				        		if (FM.CT.weaponFireMode == 0){
				        			FM.CT.weaponFireMode = 1;
				        			HUD.log(hudLogWeaponId, "Single Fire Selected");
				        		}
				        		else if (FM.CT.weaponFireMode == 1){
				        			FM.CT.weaponFireMode = 2;
				        			HUD.log(hudLogWeaponId, "Default Fire Selected");
				        		}
				        		else if (FM.CT.weaponFireMode == 2){
				        			FM.CT.weaponFireMode = 0;
				        			HUD.log(hudLogWeaponId, "Full Salvo Selected");
				        		}
				        	}
				        	break;
				        case 146:
				        	//TODO:'Weapon Release Delay'
				        	if((FM.CT.Weapons[2] != null) || (FM.CT.Weapons[3] != null))
				        	{
				        		if (FM.CT.weaponReleaseDelay == 125L){
				        			FM.CT.weaponReleaseDelay += 125L;
				        		}
				        		else if (FM.CT.weaponReleaseDelay >= 250L){
				        			FM.CT.weaponReleaseDelay += 250L;
				        		}
				        		if (FM.CT.weaponReleaseDelay > 1000L){
				        			FM.CT.weaponReleaseDelay = 125L;
				        		}
				        		HUD.log(hudLogWeaponId, "Release Delay: "+((float)FM.CT.weaponReleaseDelay/1000)+" sec");
				        	}
				        	break;
				        case 138:
...
// PAS++
                    case 53: // '5'
                        if(FM.CT.bHasFlapsControl)
                            if(!super.SetFlapsHotKeys(1, FM))	// if custom functions not present
                                SetFlapsHotKeys(1, FM);		// use std
                        break;
                    case 52: // '4'
                        if(FM.CT.bHasFlapsControl)
                            if(!super.SetFlapsHotKeys(0, FM))	// as above
...
// PAS-- TAK--
	...
	public void createPilotHotKeys()
	{
		...
		HotKeyCmdEnv.addCmd(new HotKeyCmdFire("3advanced20", "Open/Close Bay Doors", 131, 400));
		HotKeyCmdEnv hotkeycmdenv_187_ = hotkeycmdenv;
		HotKeyCmdEnv.addCmd(new HotKeyCmdFire("3advanced21", "Toggle Fuel Dump", 132, 401));
		//-----------------------------------------------------------------------------
// PAS++
		HotKeyCmdEnv hotkeycmdenv_220_ = hotkeycmdenv;
        		HotKeyCmdEnv.addCmd(new HotKeyCmdFire("3advanced22", "brakesright", 144, 351));
		HotKeyCmdEnv hotkeycmdenv_221_ = hotkeycmdenv;
        		HotKeyCmdEnv.addCmd(new HotKeyCmdFire("3advanced23", "brakesleft", 145, 352));
// PAS--
		...
		HotKeyCmdEnv.addCmd(new HotKeyCmdFire("4basic6", "Toggle Weapon System", 135, 404));
		HotKeyCmdEnv hotkeycmdenv_196_ = hotkeycmdenv;
		HotKeyCmdEnv.addCmd(new HotKeyCmdFire("4basic7", "Deploy Countermeasures", 136, 405));
		HotKeyCmdEnv hotkeycmdenv_197_ = hotkeycmdenv;
		HotKeyCmdEnv.addCmd(new HotKeyCmdFire("4basic8", "Weapon Salvo Size", 137, 406));
		HotKeyCmdEnv hotkeycmdenv_222_ = hotkeycmdenv;
		HotKeyCmdEnv.addCmd(new HotKeyCmdFire("4basic9", "Weapon Release Delay", 146, 353));
		//-----------------------------------------------------------------------------
		HotKeyCmdEnv hotkeycmdenv_198_ = hotkeycmdenv;
		HotKeyCmdEnv.addCmd(new HotKeyCmd(false, "$$+SIGHTCONTROLS", "4basic91")
		{
		});
...
// PAS++
// flaps control as a separate method providing custom flap angles for selected aircraft
// to be defined in AircraftHotKeysAlt.class
