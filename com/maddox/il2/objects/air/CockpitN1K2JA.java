...
public class CockpitN1K2JA extends com.maddox.il2.objects.air.CockpitPilot
{
    ...
    public void reflectWorldToInstruments(float f)
    {
        ...
        resetYPRmodifier();
        xyz[1] = cvt(fm.CT.getCockpitDoor(), 0.1F, 0.99F, 0.0F, 0.61F);
        mesh.chunkSetLocate("Canopy", xyz, ypr);
        resetYPRmodifier();
        xyz[2] = cvt(fm.CT.getCockpitDoor(), 0.0F, 0.1F, 0.0F, 0.0132F);
        mesh.chunkSetLocate("CanopyOpenRodL", xyz, ypr);
        xyz[2] = cvt(fm.CT.getCockpitDoor(), 0.0F, 0.1F, 0.0F, -0.0128F);
        mesh.chunkSetLocate("CanopyOpenRodR", xyz, ypr);
        mesh.chunkSetAngles("GSDimmArm", cvt(interp(setNew.dimPosition, setOld.dimPosition, f), 0.0F, 1.0F, 64F, 0.0F), 0.0F, 0.0F);
        resetYPRmodifier();
        xyz[2] = cvt(interp(setNew.dimPosition, setOld.dimPosition, f), 0.0F, 1.0F, -0.0594F, 0.0F);
        ...
        float f1 = pictTE;
        if(f1 < 0.0F)
            xyz[1] = cvt(f1, -0.25F, 0.0F, -0.02305F, 0.0F);
        else
            xyz[1] = cvt(f1, 0.0F, 0.5F, 0.0F, 0.04985F);
        mesh.chunkSetLocate("NeedElevTrim", xyz, ypr);
