...
//            RealFlightModel, AircraftState, Mass
...
public class Controls {
	...
	public void setStepControl(float f, int i) {
		if (!bUseElectricProp) {
			...
			if (!getStepControlAuto(i))
				HUD.log(AircraftHotKeys.hudLogPowerId, "PropPitch", new Object[] { new Integer(Math.round(getStepControl(i) * 100F)) });
		}
	}
	...
	public void update(float f, float f_10_, EnginesInterface enginesinterface, boolean bool, boolean bool_11_) {
		...
		f_13_ *= Sensitivity;
		f_14_ *= Sensitivity;
		if (Elevators >= 0.0F && !(((Interpolate) (FM)).actor instanceof SU_26M2))
			f_14_ = f_13_;
		if (!bool_11_)
		...
		if (StabilizerControl) {
			...
			tmpV3d.set(FM.Vwld);
			tmpV3d.normalize();
			float f_16_ = (float) (-500D * (((Tuple3d) (tmpV3d)).z - 0.001D));
			if (f_16_ > 0.8F)
				f_16_ = 0.8F;
			if (f_16_ < -0.8F)
				f_16_ = -0.8F;
			ElevatorControl = (f_16_ - 0.2F * FM.Or.getTangage() - 0.05F * FM.AOA) + 25F * (float) ((Tuple3d) (FM.getW())).y;
		...
		if (tick != Time.tickCounter()) {
			...
			CTL = (byte) ((GearControl > 0.5F ? 1 : 0) | (FlapsControl > 0.2F ? 2 : 0) | (BrakeControl > 0.2F ? 4 : 0) | (RadiatorControl > 0.5F ? 8 : 0) | (BayDoorControl > 0.5F ? 0x10 : 0)
					| (AirBrakeControl > 0.5F ? 0x20 : 0) | (DragChuteControl > 0.5F ? 0x40 : 0) | (RefuelControl > 0.5F ? 0x80 : '\0') | (VarWingControl > 0.5F ? 0x100 : '\0') | (BlownFlapsControl > 0.5F ? 0x200
					: '\0'));
			WCT &= 0xfc;
			TWCT &= 0xfc;
			int i = 0;
			...
			if ((double) Time.speed() == 0.5D)
				delay *= 2L;
			else if ((double) Time.speed() == 0.25D)
				delay *= 4L;
			else
							...
							for (int i_22_ = 0; i_22_ < Weapons[i_19_].length; i_22_ += 2) {
								...
								if (bHasBayDoors && Weapons[i_19_][i_22_].getHookName().startsWith("_BombSpawn")) {
									...
									if (bHasBayDoors && Weapons[i_19_][i_22_].getHookName().startsWith("_InternalRock") && BayDoorControl == 0.0F)
										break;
									if (weaponFireMode == 2 || (Weapons[i_19_][i_22_] instanceof RocketGun4andHalfInch) || (Weapons[i_19_][i_22_] instanceof RocketGun) && iToggleRocketSide == LEFT
											|| (Weapons[i_19_][i_22_] instanceof RocketBombGun) && iToggleRocketSide == LEFT || (Weapons[i_19_][i_22_] instanceof BombGun) && iToggleBombSide == LEFT)
										Weapons[i_19_][i_22_].shots(i_20_);
									if (Weapons[i_19_][i_22_].getHookName().startsWith("_BombSpawn"))
										BayDoorControl = 1.0F;
								}
								if ((Weapons[i_19_][i_22_] instanceof BombGun) && weaponFireMode == 0 || (Weapons[i_19_][i_22_] instanceof RocketGun) && !(Weapons[i_19_][i_22_] instanceof MissileGun)
										&& weaponFireMode == 0) {
									Weapons[i_19_][i_22_].shots(i_20_);
									continue;
								}
							...
							for (int i_23_ = 1; i_23_ < Weapons[i_19_].length; i_23_ += 2) {
								...
								if (bHasBayDoors && Weapons[i_19_][i_23_].getHookName().startsWith("_BombSpawn")) {
									...
									if (bHasBayDoors && Weapons[i_19_][i_23_].getHookName().startsWith("_InternalRock") && BayDoorControl == 0.0F)
										break;
									if (weaponFireMode == 2 || (Weapons[i_19_][i_23_] instanceof RocketGun4andHalfInch) || (Weapons[i_19_][i_23_] instanceof RocketGun) && iToggleRocketSide == RIGHT
											|| (Weapons[i_19_][i_23_] instanceof RocketBombGun) && iToggleRocketSide == LEFT || (Weapons[i_19_][i_23_] instanceof BombGun) && iToggleBombSide == RIGHT)
										Weapons[i_19_][i_23_].shots(i_20_);
								}
								if ((Weapons[i_19_][i_23_] instanceof BombGun) && weaponFireMode == 0 || (Weapons[i_19_][i_23_] instanceof RocketGun) && !(Weapons[i_19_][i_23_] instanceof MissileGun)
										&& weaponFireMode == 0) {
									Weapons[i_19_][i_23_].shots(i_20_);
									continue;
								}
	...
	public void doSetWeaponFireMode(int theWeaponFireMode) {
		weaponFireMode = theWeaponFireMode;
	}
	...
	public void doSetWeaponReleaseDelay(long theWeaponReleaseDelay) {
		weaponReleaseDelay = theWeaponReleaseDelay;
	}

	public void doChangeWeaponReleaseDelay(long theWeaponReleaseDelayOffset) {
		weaponReleaseDelay += theWeaponReleaseDelayOffset;
	}
	...
	public void toggleWeaponReleaseDelay(int hudLogWeaponId) {
		...
		else if (weaponReleaseDelay >= 250L)
			doChangeWeaponReleaseDelay(250L);
		if (weaponReleaseDelay > 1000L)
			doSetWeaponReleaseDelay(125L);
