...
class OrderAttack_Bombers extends com.maddox.il2.game.order.Order
{
        ...
        for(int i = 0; i < CommandSet().length; i++)
        {
            ...
            if(pilot.Group != null)
            {
                if(com.maddox.il2.game.order.OrdersTree.curOrdersTree.alone())
                    if((pilot.Group.grTask != 3 || pilot.Group.aTargetPreference != 8) && ((com.maddox.il2.ai.air.Maneuver)Player().FM).Group == pilot.Group)
                    {
                ...
                if(airgroup1 != null)
                {
                    ...
                    if(isEnableVoice() && aircraft != Player() && pilot.canAttack())
                        if(aircraft.getWing() == Player().getWing() || aircraft.aircIndex() == 0)
                            com.maddox.il2.objects.sounds.Voice.speakAttackBombers(aircraft);
                        else
