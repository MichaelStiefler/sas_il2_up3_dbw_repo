...
public class CmdChannel extends com.maddox.rts.Cmd
{
    ...
    {
        boolean flag = com.maddox.rts.Cmd.exist(map, "SOCKET");
        int i = -1;
        if(com.maddox.rts.Cmd.nargs(map, "_$$") == 1)
        {
            i = com.maddox.rts.Cmd.arg(map, "_$$", 0, -1);
            if(i == -1)
            {
            ...
            if(com.maddox.rts.Cmd.exist(map, "SPEED") || com.maddox.rts.Cmd.exist(map, "TIMEOUT") || com.maddox.rts.Cmd.exist(map, "STAT"))
            {
                if(com.maddox.rts.Cmd.nargs(map, "SPEED") == 1)
                {
                    double d = (double)com.maddox.rts.Cmd.arg(map, "SPEED", 0, 1000) / 1000D;
                    netchannel.setMaxSpeed(d);
                }
                if(com.maddox.rts.Cmd.nargs(map, "TIMEOUT") == 1)
                {
                    int j = com.maddox.rts.Cmd.arg(map, "TIMEOUT", 0, 131);
                    netchannel.setMaxTimeout(j * 1000);
                }
                if(com.maddox.rts.Cmd.nargs(map, "STAT") == 1)
                {
                    int k = com.maddox.rts.Cmd.arg(map, "STAT", 0, -1) * 1000;
                    if(k <= 0)
                    {
