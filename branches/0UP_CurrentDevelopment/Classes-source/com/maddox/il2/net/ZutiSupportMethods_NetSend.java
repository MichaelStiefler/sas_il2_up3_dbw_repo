...
public class ZutiSupportMethods_NetSend
{
	...
	public static final byte UPDATE_NET_PLACE1 = 79;
	public static final byte FRONT_MARKER = 80;
	public static final byte FRONT_REFRESH = 81;
	public static final byte RESOURCES_INFO = 82;
	...
	public static void processCockpitChangeRequest(NetUser sender, String acName, int requestedPosition, boolean isPilot)
	{
		try
		{
			...
			System.out.println("ZSM_NS - User >" + sender.uniqueName() + "< moved to cockpit nr. " + requestedPosition + " in AC " + acName);
			NetMsgGuaranted netmsgguaranted = null;
	...
	public static void returnRRRResources_Fuel(float unusedFuel, Point3d pos)
	{
		try
		{
			...
			if( ZutiSupportMethods.isOnZAP(pos) > -1 )
			...
			if (netuser == null || Main.cur().netServerParams == null || Main.cur().netServerParams.isMaster() || Mission.isSingle() )
			{
				...
				{
					case 1:
						if( Mission.MDS_VARIABLES().enabledResourcesManagement_RedSide )
						{
						...
						break;
					case 2:
						if( Mission.MDS_VARIABLES().enabledResourcesManagement_BlueSide )
						{
	...
	public static void reportSpawnResources(Aircraft aircraft)
	{
		try
		{
			...
			System.out.println("=============REQUESTING PAYLOAD============");
			...
			long cargo = ZutiSupportMethods_GameOrder.getNrOfCargoCratesForAircraft(aircraft);

			System.out.println("  Bullets: " + bullets);
			System.out.println("  Rockets: " + rockets);
			...
			{
				//Single player mode, RRR must also work!
				ZutiSupportMethods_NetSend_ToClients.getAvailablePayloadResourcesAndSendThemToUser(bullets, bombs, rockets, fuel, cargo, null);
				System.out.println("Single player game - payload resources updated!");
				return;
			}
			...
			if (Main.cur().netServerParams.isMaster())
			{
				...
				ZutiSupportMethods_NetSend_ToClients.getAvailablePayloadResourcesAndSendThemToUser(bullets, bombs, rockets, fuel, cargo, user);
				System.out.println("Online game - payload resources forwarded to user: " + user);
			}
			System.out.println("===========================================");
		}
		catch (Exception ex)
	 ...
	 * !!!!!!!!!!!!!METHOD DISABLED - USED LOCAL PROCESSING INSTEAD!!!!!!!!!!!!!
	 */
	public static void requestNetPlacesUpdate_DISABLED()
	{
		/*
		try
		{
	...
	/**
	 * Send request to server that we want to get information about resources at current position.
	 * 
	 * @param bullets
	 */
	public static void requestResourcesInformation(Point3d position)
	{
		if( ZutiSupportMethods_GUI.ZUTI_RESOURCES_INFORMATION == 0 || ZutiSupportMethods_GUI.ZUTI_RESOURCES_INFORMATION + 10000 < Time.current() )
		{
			ZutiSupportMethods_GUI.ZUTI_RESOURCES_INFORMATION = Time.current();
			...
			try
			{
				if (Main.cur().netServerParams == null || Mission.isSingle())
				{
					String[] result = ZutiSupportMethods_ResourcesManagement.getResourcesInformationForLocation(position.x, position.y);
					String line = null;
				...
				if (Main.cur().netServerParams.masterChannel() != null)
				{
					//Send to server
					NetMsgGuaranted netmsgguaranted = new NetMsgGuaranted();
					netmsgguaranted.writeByte(RESOURCES_INFO);
					netmsgguaranted.writeDouble(position.x);
					netmsgguaranted.writeDouble(position.y);
		...
		{
			System.out.println("You can only request resources info every few seconds!");
		}
	}
	
	/**
	 * Send user information about resources for specified location.
	 * 
	 * @param netuser
	 * @param x
	 * @param y
	 */
	public static void resourcesInformation(NetUser netuser, double x, double y)
	{
		try
		{
			if (netuser == null)
			...
			String[] result = ZutiSupportMethods_ResourcesManagement.getResourcesInformationForLocation(x, y);
			...
			for (int i = 0; i < result.length; i++)
			{
				...
				netmsgguaranted = new NetMsgGuaranted();
				netmsgguaranted.writeByte(RESOURCES_INFO);
				netmsgguaranted.write255(line);

				NetEnv.host().postTo(netuser.masterChannel(), netmsgguaranted);
			}
			System.out.println("ZSM_NS - resources info sent to user >" + netuser.uniqueName() + "<.");
		}
		catch (IOException ex)
		{
			ex.printStackTrace();
