...
public class Front
{
    ...
    {
        if(aircraft == null || aircraft.FM == null)
            return;
    ...
    public static boolean isCaptured(int i, double d, double d1)
    {
        ...
        do
        {
            ...
            double d12 = (d5 + d7) * 0.5D;
            int k = com.maddox.il2.ai.Front.army(d10, d12);
            if(k == i)
            {
                d3 = d10;
                d5 = d12;
        ...
        for(int j1 = 0; j1 < l; j1++)
        {
            ...
            double d13 = (point3d.x - d) * (point3d.x - d) + (point3d.y - d1) * (point3d.y - d1);
            if(d13 < 1000000D)
                return true;
        }

        double d9 = java.lang.Math.sqrt(d2) / 50000D;
        double d11 = d + d1;
        if(d11 < 0.0D)
            d11 = -d11;
        d11 -= (int)d11;
        return 0.59999999999999998D + d9 * 0.5D + (d11 - 0.5D) >= 0.5D;
    }
    ...
    private void tilesUpdate()
    {
        ...
        for(int k1 = 0; k1 < k; k1++)
        {
            ...
            double d5 = marker1.x - d2;
            double d7 = marker1.y - d1;
            marker1._d2 = d5 * d5 + d7 * d7;
            if(d3 > marker1._d2)
                d3 = marker1._d2;
        }
        ...
        k = markers.size();
        double d4 = camWorldYOffset - 0.5D * d;
        double d6 = camWorldXOffset - 0.5D * d;
        int j2 = 0;
        int k2 = 0;
        l = 0;
        for(int l2 = 0; l2 < j; l2++)
        {
            double d8 = d4 + (double)l2 * d;
            for(int k4 = 0; k4 < i; k4++)
            {
                double d9 = d6 + (double)k4 * d;
                double d10 = d3;
        ...
        for(int j3 = 1; j3 < j - 1; j3++)
        {
            for(int l3 = 1; l3 < i - 1; l3++)
            {
                int j4 = j3 * i + l3;
                byte byte0 = mask2[j4];
                if(byte0 == 0)
                ...
                if(mask2[j4 + 1] == byte0)
                {
                    if(mask2[j4 + i] != mask2[(j4 - i) + 1] && mask2[j4 - i] != mask2[j4 + i + 1])
                        l4 |= 1;
                } else
                if(mask2[j4 + i + 1] == byte0 && mask2[j4 + i] != byte0 && mask2[j4 + i] != mask2[j4 + 1])
                    l4 |= 2;
                if(mask2[j4 + i] == byte0)
                {
                    if(mask2[j4 - 1] != mask2[j4 + i + 1] && mask2[(j4 + i) - 1] != mask2[j4 + 1])
                        l4 |= 4;
                } else
                if(mask2[(j4 + i) - 1] == byte0 && mask2[j4 - 1] != byte0 && mask2[j4 - 1] != mask2[j4 + i])
                    l4 |= 8;
                mask[j4] = (byte)l4;
            }
