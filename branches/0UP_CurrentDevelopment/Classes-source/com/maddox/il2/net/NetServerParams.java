...
public class NetServerParams extends NetObj implements NetUpdate
{
	...
	private int checkFirst(int i)
	{
		if (i != 0)
		{
			...
			if( NetServerParams.DEBUG )
				System.out.println("Check sum before CRT1 checks: " + l);
			...
			if( NetServerParams.DEBUG )
				System.out.println("Check after java dll files and before AC class files checks: " + l);
			...
			for (int i_22_ = 0; i_22_ < arraylist.size(); i_22_++)
			{
				...
				if( propertyName != null && propertyName.indexOf("C-47B.fmd") < 0 && propertyName.indexOf("Yak-9P.fmd") < 0 && 
					propertyName.indexOf("Bf-") < 0 )
				{					
					l = FlightModelMain.finger(l, propertyName);

					if( NetServerParams.DEBUG )
						System.out.println("Check after AC class " + propertyName + " check: " + l);
				}
			}
			...
			if( NetServerParams.DEBUG )
				System.out.println("Check sum after currentTime checks: " + l);
