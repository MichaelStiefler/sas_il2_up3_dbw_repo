...
public class CockpitMig_17Radar extends CockpitPilot {
  ...
  public void draw() {
    try {
      ...
      if (!Mission.isNet() && Actor.isValid(ownaircraft) && Actor.isAlive(ownaircraft)) {
        int ti = (int) ((Time.current() % 1000L) / 500L);
        if (ti != to) {
          to = ti;
        ...
        for (int j = 0; j < i; j++) {
          ...
          mesh.render();
          if (!mesh.isChunkVisible(t)) {
            mesh.chunkVisible(t, true);
          }
        }
  ...
  public void CockpitBF_110G_Gunner() {
    ...
    setNightMats(false);
    interpPut(new CockpitPilot.Interpolater(), null, Time.current(), null);
