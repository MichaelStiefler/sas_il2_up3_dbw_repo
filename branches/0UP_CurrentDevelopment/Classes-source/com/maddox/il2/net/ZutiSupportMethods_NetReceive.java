...
public class ZutiSupportMethods_NetReceive
{
	...
	public static boolean processReceivedMessage(NetUser sender, NetMsgInput netmsginput, byte messageId)
	{
		try
		{
			...
			{			
				...
				/*
				 * DISABLED - Call to UpdateNetUsersCrewPositions is done directly by the GUI button.
				 * 
				...
				*/
				...
				case ZutiSupportMethods_NetSend.FRONT_REFRESH:
					...
					if( !Main.cur().netServerParams.isMaster() )
						frontRefresh();
					return true;
				case ZutiSupportMethods_NetSend.RESOURCES_INFO:
					if( Main.cur().netServerParams.isMaster() )
					{
						double x = netmsginput.readDouble();
						double y = netmsginput.readDouble();
						ZutiSupportMethods_NetSend.resourcesInformation(sender, x, y);
					}
					else
						processResourcesInformation(netmsginput);
					return true;
	...
	private static void processResourcesInformation(NetMsgInput netmsginput)
	{
		try
		{
			System.out.println(netmsginput.read255());
		}
		catch(Exception ex){ex.printStackTrace();}
	}
}