...
//  DecimalFormat twoPlaces = new DecimalFormat("+000.00;-000.00"); // only required for debugging
  ...
  private int iTargetType = TARGET_AIR;
...
//    this.tStart = Time.current() - (long)Property.floatValue(this.getClass(), "warmupTime", 0.0F);
//    EventLog.type("MissileInit 1");
  }
...
//    EventLog.type("MissileInit 2");
  ...
  private void getMissileProperties() {
    ...
    this.iTargetType = Property.intValue(localClass, "targetType", TARGET_AIR);
  ...
  public void createSmokeTest() {
    Hook localHook = null;
    String str = Property.stringValue(this.getClass(), "smoke", null);
    if (str != null) {
      if (localHook == null) {
    ...
    try {
      if (this.ownerIsAI()) {
        if (getOwner() instanceof TypeGuidedMissileCarrier) {
          this.victim = ((TypeGuidedMissileCarrier) getOwner()).getMissileTarget();
          this.pVictimOffset = ((TypeGuidedMissileCarrier) getOwner()).getMissileTargetOffset();
          safeVictimOffset.set(pVictimOffset);
