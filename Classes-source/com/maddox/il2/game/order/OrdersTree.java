...
public class OrdersTree
{
	...
	static Orders MDSOrders	= new Orders(new Order[] 
    {
			new Order("mds.MDS"), 
			new Order("mds.vectoring", ToNavigationTeam), 
			new Order("mds.turrets", ToTurrets),
			null,
			new Order("mds.kickGunner", ToGunnersTeam),
			new Order("mds.transferControl", ToTransferTeam),
			null,
			new ZutiOrder_ResourcesInformation(),
			new Order("mds.RRR2", ToReloadingTeam), 
			new Order("mds.unload", ToUnloadingTeam), 
			new OrderBack()				
	});
	//--------------------------------------------------------------------------------------------------------------------	
	static Orders ToGroundControl = new Orders(new com.maddox.il2.game.order.Order[] {
            new Order("GroundControl"), 
            new OrderRequest_Assistance(), 
            new OrderVector_To_Home_Base(), 
            new OrderVector_To_Target(), 
            new OrderRequest_For_Landing(), 
            new OrderRequest_For_Takeoff(), 
            new OrderRequest_For_RunwayLights(), 
            null,
            //TODO: Added by |ZUTI|
            new Order("mds.MDS", MDSOrders),
            null,
            new OrderBack()
        });
