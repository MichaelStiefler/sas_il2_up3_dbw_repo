...
public class Squares
{
    ...
    {
        java.lang.String s1 = "Zero Square processed from " + sectfile.toString();
        ...
        if(f1 < 0.9F || f1 > 1.1F)
        {
            if(com.maddox.il2.ai.World.cur().isDebugFM())
                java.lang.System.out.println("Error in flightmodel " + sectfile.toString() + ": (wing square) != (sum of squares*2)");
            if(f1 > 1.0F)
    ...
    public void computeParasiteDrag(com.maddox.il2.fm.Controls controls, com.maddox.il2.ai.BulletEmitter abulletemitter[][])
    {
        ...
        for(int i = 0; i < abulletemitter.length; i++)
            if(abulletemitter[i] != null && abulletemitter[i].length > 0)
            {
                ...
                {
                    if((abulletemitter[i][j] instanceof com.maddox.il2.objects.weapons.BombGun) && abulletemitter[i][j].haveBullets() && abulletemitter[i][j].getHookName().startsWith("_External") && dragParasiteCx < 0.704F)
