...
public class AIFlightModel extends com.maddox.il2.fm.FlightModel
{
    ...
    public void update(float f)
    {
        ...
        if(isTick(44, 0))
        {
            ...
            if(((com.maddox.il2.ai.air.Pilot)((com.maddox.il2.objects.air.Aircraft)actor).FM).get_task() == 7 || ((com.maddox.il2.ai.air.Maneuver)((com.maddox.il2.objects.air.Aircraft)actor).FM).get_maneuver() == 43)
                putScareShpere();
        ...
        if(d < 0.40000000000000002D * (double)Length)
        {
            ...
            double d3 = 1.0D + 0.29999999999999999D * d1;
            Cw.z *= d2;
            Cw.x *= d3;
        }
        ...
        if(f3 > 15000F)
            f3 = 15000F;
        if(((com.maddox.il2.ai.air.Maneuver)((com.maddox.il2.objects.air.Aircraft)actor).FM).get_maneuver() != 20)
        {
            float f4 = f1 - f2;
            ...
            } else
            {
                if(!Gears.onGround())
                {
                    ...
                    if(((com.maddox.il2.ai.air.Maneuver)((com.maddox.il2.objects.air.Aircraft)actor).FM).get_maneuver() == 44 && (AOA > 15F || AOA < -12F))
                    {
                        if(f4 > 0.0F && W.z > 0.0D)
        ...
        if(W.lengthSquared() > 36D)
            W.scale(6D / W.length());
        double d4 = 0.10000000000000001D + (double)Sq.squareWing;
        if(d4 < 1.0D)
            d4 = 1.0D;
        d4 = 1.0D / d4;
        W.x *= 1.0D - 0.12D * (0.20000000000000001D + (double)f1 + (double)f2) * d4;
        W.y *= 1.0D - 0.5D * (0.20000000000000001D + (double)Sq.liftStab) * d4;
        W.z *= 1.0D - 0.5D * (0.20000000000000001D + (double)Sq.liftKeel) * d4;
        ...
        if(!brakeShoe && (Gears.nOfPoiOnGr == 0 && Gears.nOfGearsOnGr < 3 || f11 == 0.0F || SummM.lengthSquared() > (double)(2.0F * f12) || SummF.lengthSquared() > (double)(80F * f12) || W.lengthSquared() > (double)(0.00014F * f11) || Vwld.lengthSquared() > (double)(0.09F * f11)))
        {
            ...
            for(int i1 = 0; i1 < l; i1++)
            {
                ...
                Loc.add(TmpP);
                boolean flag;
                if(com.maddox.il2.fm.FMMath.isNAN(Loc))
                    flag = false;
                GF.set(0.0D, 0.0D, 0.0D);
                GM.set(0.0D, 0.0D, 0.0D);
                if(i1 < l - 1)
