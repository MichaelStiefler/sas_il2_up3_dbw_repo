...
public class Gear
{
    ...
    public void ground(com.maddox.il2.fm.FlightModel flightmodel, boolean flag, boolean flag1)
    {
        ...
        for(int k = 0; k < pnti.length; k++)
        {
            clp[k] = false;
            if(k <= 2)
            {
                bIsGear = true;
                if(k == 0 && (!lgear || FM.CT.getGear() < 0.01F) || k == 1 && (!rgear || FM.CT.getGear() < 0.01F) || k == 2 && !cgear)
                    continue;
            } else
            {
                bIsGear = false;
            ...
            if(d < 0.0D)
            {
                if(!testPropellorCollision(k) || (isWater ? !testWaterCollision(k) : bIsGear ? !testGearCollision(k) : !testNonGearCollision(k)))
                    continue;
                clp[k] = true;
                nP++;
            } else
            {
                if(d >= 0.10000000000000001D || isWater || bIsGear || !testNonGearCollision(k))
                    continue;
                clp[k] = true;
                nP++;
            }
        ...
        if(bIsMaster)
        {
            ...
            if((com.maddox.il2.game.Mission.isCoop() || com.maddox.il2.game.Mission.isDogfight()) && !com.maddox.il2.game.Mission.isServer() && com.maddox.il2.engine.Actor.isAlive(FM.brakeShoeLastCarrier) && com.maddox.rts.Time.current() < 60000L)
            ...
            if(FM.actor == com.maddox.il2.ai.World.getPlayerAircraft() && FM.Loc.z - com.maddox.il2.engine.Engine.land().HQ_Air(FM.Loc.x, FM.Loc.y) < 2D && !bTheBabysGonnaBlow)
            {
                for(int j = 0; j < FM.CT.Weapons.length; j++)
                {
                    ...
                    for(int k = 0; k < FM.CT.Weapons[j].length; k++)
                        if(((FM.CT.Weapons[j][k] instanceof com.maddox.il2.objects.weapons.BombGun) || (FM.CT.Weapons[j][k] instanceof com.maddox.il2.objects.weapons.RocketGun) || (FM.CT.Weapons[j][k] instanceof com.maddox.il2.objects.weapons.RocketBombGun)) && FM.CT.Weapons[j][k].haveBullets() && FM.getSpeed() > 38F && FM.CT.Weapons[j][k].getHookName().startsWith("_External"))
    ...
    public double plateFriction(com.maddox.il2.fm.FlightModel flightmodel)
    {
        ...
        currentBornPlace = com.maddox.il2.net.BornPlace.getCurrentBornPlace(flightmodel.Loc.x, flightmodel.Loc.y);
    ...
    public void zutiCheckPlaneForSkisAndWinterCamo(java.lang.String s1)
    {
        for(int i = 0; i < ZUTI_SKIS_AC_CLASSES.length; i++)
            if(s1.endsWith(ZUTI_SKIS_AC_CLASSES[i]))
            {
                if("WINTER".equals(com.maddox.il2.engine.Engine.land().config.camouflage))
    ...
    private com.maddox.il2.net.BornPlace currentBornPlace;
    private boolean zutiAlreadyReloaded;
    private static java.lang.String ZUTI_SKIS_AC_CLASSES[] = {
