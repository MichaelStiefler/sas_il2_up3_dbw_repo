...
label0:
    ...
    protected void doMouseAbsMove(float f, float f1, float f2)
    {
        ...
        {
            com.maddox.gwindow.GWindow gwindow = (com.maddox.gwindow.GWindow)mouseListeners.get(i);
            if(!gwindow.isWaitModal())
                gwindow.msgMouseMove(true, f, f1);
        }

        if(bDoCheckKeyFocusWindow)
            doCheckKeyFocusWindow();
        com.maddox.gwindow.GWindow gwindow1;
        if(!bMouseCapture)
        {
            gwindow1 = findWindowUnder(f, f1);
            if(gwindow1.isWaitModal())
                gwindow1 = modalWindow;
        } else
        {
            gwindow1 = mouseWindow;
        }
        if(gwindow1 != mouseWindow)
        {
            mouseOldWindow = mouseWindow;
            mouseWindow = gwindow1;
    ...
    protected void doMouseButton(int i, boolean flag, float f, float f1)
    {
        for(int j = 0; j < mouseListeners.size(); j++)
        {
            com.maddox.gwindow.GWindow gwindow = (com.maddox.gwindow.GWindow)mouseListeners.get(j);
            if(!gwindow.isWaitModal())
                gwindow.msgMouseButton(true, i, flag, f, f1);
        }
        ...
        if(!bMouseCapture)
        {
            com.maddox.gwindow.GWindow gwindow1 = findWindowUnder(f, f1);
            if(gwindow1.isWaitModal())
                gwindow1 = modalWindow;
            if(gwindow1 != mouseWindow)
            {
