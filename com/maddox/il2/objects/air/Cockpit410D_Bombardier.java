...
public class Cockpit410D_Bombardier extends CockpitPilot
{
  ...
  public Cockpit410D_Bombardier() { super("3DO/Cockpit/Ar-234B-2-Bombardier/410D.him", "he111");
  ...
  {
    if (this.bBAiming) {
      this.mesh.chunkSetAngles("zAngleMark", -floatindex(cvt(((ME_410D)aircraft()).fSightCurForwardAngle, 7.0F, 140.0F, 0.7F, 14.0F), angleScale), 0.0F, 0.0F);

      boolean bool = ((ME_410D)aircraft()).fSightCurReadyness > 0.93F;
  ...
  class Interpolater extends InterpolateRef
  {
    ...
    public boolean tick()
    {
      float f1 = ((ME_410D)Cockpit410D_Bombardier.this.aircraft()).fSightCurForwardAngle;

      float f2 = ((ME_410D)Cockpit410D_Bombardier.this.aircraft()).fSightCurSideslip;

      Cockpit410D_Bombardier.this.mesh.chunkSetAngles("BlackBox", 0.0F, -f2, f1);
      if ((Cockpit410D_Bombardier.this.bEntered) && (Cockpit410D_Bombardier.this.bBAiming)) {
        HookPilot localHookPilot = HookPilot.current;
        localHookPilot.setSimpleAimOrient(Cockpit410D_Bombardier.this.aAim + f2, Cockpit410D_Bombardier.this.tAim + f1, 0.0F);
      }
      float f3 = Cockpit410D_Bombardier.this.fm.getAltitude();
      float f4 = (float)(-(Math.abs(Cockpit410D_Bombardier.this.fm.Vwld.length()) * Math.sin(Math.toRadians(Math.abs(Cockpit410D_Bombardier.this.fm.Or.getTangage())))) * 0.1018999963998795D);

      f4 += (float)Math.sqrt(f4 * f4 + 2.0F * f3 * 0.1019F);

      float f5 = Math.abs((float)Cockpit410D_Bombardier.this.fm.Vwld.length()) * (float)Math.cos(Math.toRadians(Math.abs(Cockpit410D_Bombardier.this.fm.Or.getTangage())));

      float f6 = f5 * f4 + 10.0F - 10.0F;
      Cockpit410D_Bombardier.access$402(Cockpit410D_Bombardier.this, 90.0F - Math.abs(Cockpit410D_Bombardier.this.fm.Or.getTangage()) - (float)Math.toDegrees(Math.atan(f6 / f3)));
