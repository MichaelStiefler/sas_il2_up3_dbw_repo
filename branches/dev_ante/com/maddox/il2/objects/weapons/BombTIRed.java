...
public class BombTIRed extends Bomb
{
  private long t1;
  private int charge;
    ...
    if (this.charge < 1.01D) {
      ...
      if (Actor.isValid(localActor)) {
        ...
        for (int i = 0; i < 60; i++) {
          ...
          BombTiRd localBombTiRd = new BombTiRd();
          localBombTiRd.pos.setUpdateEnable(true);
          localBombTiRd.pos.setAbs(localPoint3d, localOrient);
          localBombTiRd.pos.reset();
          localBombTiRd.start();
          localBombTiRd.setOwner(localActor, false, false, false);
          localBombTiRd.setSpeed(localVector3d1);
        }
  ...
  static
  {
    Class localClass = BombTIRed.class;
