// Decompiled by DJ v3.10.10.93 Copyright 2007 Atanas Neshkov  Date: 20/07/2011 3:28:05 PM
...
//            Controls, Gear, Mass, FlightModel, 
//            Autopilotage, Pitot
    ...
    private void doSetBleedingPilot(int paramInt1, int paramInt2, Actor paramActor)
    {
        if(Mission.isSingle() || ((SndAircraft) (aircraft)).FM.isPlayers() && !Mission.isServer() || Mission.isNet() && Mission.isServer() && !aircraft.isNetPlayer())
        {
            ...
            if(astateBleedingNext[paramInt1] == 0L)
            {
                ...
                {
                    int i = paramInt1 == astatePlayerIndex && !World.isPlayerDead() ? 1 : 0;
                    if(paramInt2 > 10)
        ...
        if(bIsMaster)
        {
            ...
            if(bDumpFuel && aircraft instanceof TypeFuelDump)
            	if(aircraft.FM.M.fuel > ((TypeFuelDump) aircraft).getFuelReserve())
                	aircraft.FM.M.fuel -= ((TypeFuelDump) aircraft).getFlowRate() * paramFloat;
                else
        	...
        	//TODO: Edit allowing F-86D to keep rocket tray door open indefinitely
            if(!((actor instanceof TypeFighter) && (actor instanceof TypeBayDoor)))
            {
            	//TODO: Don't disable this statement and bailout. Else, you won't be able to bailout!
