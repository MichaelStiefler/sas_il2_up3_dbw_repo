  ...
  implements Serializable
{
  ...
  public void setLeftUpperCorner(Point3d paramPoint3d)
  {
    this.leftUpperCorner.set(paramPoint3d);
  }

  public boolean findPlaceForAirPlane(CellAirPlane paramCellAirPlane)
  {
    if (!paramCellAirPlane.checkAirFieldSize(this))
      return false;
    if (!paramCellAirPlane.checkAirFieldCapacity(this))
      return false;
  ...
  public boolean findPlaceForAirPlaneCarrier(CellAirPlane paramCellAirPlane)
  {
    if (!paramCellAirPlane.checkAirFieldSizeCarrier(this))
      return false;
    if (!paramCellAirPlane.checkAirFieldCapacity(this))
      return false;
    for (int i = 0; i <= getHeight() - (paramCellAirPlane.getHeight() + 1); i++)
    {
      for (int j = 0; j <= getWidth() - paramCellAirPlane.getWidth(); j++) {
        if (!isThereFreePlaceCarrier(paramCellAirPlane, j, i))
          continue;
        this.resX = j;
        this.resY = i;
        return true;
  ...
  public boolean isThereFreePlace(CellAirPlane paramCellAirPlane, int paramInt1, int paramInt2)
  {
    ...
    for (int i = paramInt2; i < paramInt2 + paramCellAirPlane.getHeight(); i++)
    {
      ...
      {
        if ((getCells()[j][i] != null) && (paramCellAirPlane.getCells()[(j - paramInt1)][(i - paramInt2)] != null))
          return false;
        if (paramCellAirPlane.getCells()[(j - paramInt1)][(i - paramInt2)] == null)
          continue;
        for (int k = i; k < getHeight(); k++) {
          if ((getCells()[j][k] != null) && (getCells()[j][k] != this)) {
            return false;
          }
        }
      }
  ...
  public boolean isThereFreePlaceCarrier(CellAirPlane paramCellAirPlane, int paramInt1, int paramInt2)
  {
    if (getCells() == null)
      return false;
    for (int i = paramInt2; i < paramInt2 + paramCellAirPlane.getHeight() + 1; i++)
    {
      for (int j = paramInt1; j < paramInt1 + paramCellAirPlane.getWidth(); j++)
      {
        if (getCells()[j][i] != null) {
          return false;
        }
      }
    }
  ...
  public void placeAirPlaneCarrier(CellAirPlane paramCellAirPlane, int paramInt1, int paramInt2)
  {
    if (getCells() == null)
      return;
    paramCellAirPlane.setWorldCoordinates(getXCoordinate() + paramInt1 * getCellSize(), getYCoordinate() + paramInt2 * getCellSize());
    for (int i = 0; i < paramCellAirPlane.getHeight() + 1; i++)
    {
      for (int j = 0; j < paramCellAirPlane.getWidth(); j++)
      {
        getCells()[(paramInt1 + j)][(paramInt2 + i)] = paramCellAirPlane;
  ...
  public void replaceAirPlane(CellAirPlane paramCellAirPlane, int paramInt1, int paramInt2)
  {
    ...
    for (int i = 0; i < paramCellAirPlane.getWidth(); i++)
    {
      for (int j = 0; j < paramCellAirPlane.getHeight(); j++)
        if (getCells()[i][j] == paramCellAirPlane)
          getCells()[(paramInt1 + i)][(paramInt2 + j)] = null;
    }
  }
  ...
  public Point3d getLeftUpperPoint(ArrayList paramArrayList)
  {
    ...
    for (int i = 0; i < paramArrayList.size(); i++)
    {
      ...
      if (localPoint3d2.x < d1)
        d1 = localPoint3d2.x;
      if (localPoint3d2.y > d2) {
        d2 = localPoint3d2.y;
      }
    }
