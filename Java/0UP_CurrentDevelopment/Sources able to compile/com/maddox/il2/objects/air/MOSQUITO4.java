    ...
    implements TypeBomber
{
    ...
    public void typeBomberAdjDistancePlus()
    {
        ...
        fSightCurDistance = MOSQUITO4.toMeters(fSightCurAltitude) * (float)java.lang.Math.tan(java.lang.Math.toRadians(fSightCurForwardAngle));
    ...
    public void typeBomberUpdate(float f)
    {
        ...
        if(bSightAutomation)
        {
            fSightCurDistance -= MOSQUITO4.toMetersPerSecond(fSightCurSpeed) * f;
            if(fSightCurDistance < 0.0F)
            {
                fSightCurDistance = 0.0F;
                typeBomberToggleAutomation();
            }
            fSightCurForwardAngle = (float)java.lang.Math.toDegrees(java.lang.Math.atan(fSightCurDistance / MOSQUITO4.toMeters(fSightCurAltitude)));
            calibDistance = MOSQUITO4.toMetersPerSecond(fSightCurSpeed) * floatindex(Aircraft.cvt(MOSQUITO4.toMeters(fSightCurAltitude), 0.0F, 7000F, 0.0F, 7F), calibrationScale);
            if((double)fSightCurDistance < (double)calibDistance + (double)MOSQUITO4.toMetersPerSecond(fSightCurSpeed) * java.lang.Math.sqrt(MOSQUITO4.toMeters(fSightCurAltitude) * 0.2038736F))
                bSightBombDump = true;
            if(bSightBombDump)
                if(FM.isTick(3, 0))
                {
    ...
    static 
    {
        java.lang.Class class1 = MOSQUITO4.class;
